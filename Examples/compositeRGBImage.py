...
if __name__ == "__main__":
    ...
    mtime_str = datetime.fromtimestamp(Path(__file__).stat().st_mtime).isoformat(
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="path+filename of the HARP2 Level 1C file",
    )
    parser.add_argument(
        "--save_path", type=str, required=False, help="path to save the composite image"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Normalization factor for the RGB plot",
    )
    parser.add_argument(
        "--label", type=int, default=1, help="Add label to the RGB plot"
    )
    parser.add_argument(
        "--tag", type=str, default="", help="Tag to add to the filename"
    )
    parser.add_argument(
        "--viewIndex", type=int, default=0, help="Tag to add to the filename"
    )
    parser.add_argument(
        "--dolp", type=int, default=0, help="Use Dolp to plot the RGB image"
    )
    ...
    args.label = bool(args.label)
    args.dolp = bool(args.dolp)

    # -- validate arguments
    assert os.path.exists(args.l1c_dir), "l1c_file does not exist!"
    assert args.instrument in [
        "HARP2",
        "AirHARP2",
    ], "instrument must be HARP2 or AirHARP2"
    assert args.normFactor > 0, "normFactor must be greater than 0"

    # save the figure in the same directory as the L1C file
    if args.save_path is None:
        print(
            "save_path is not provided. Saving the composite in the same directory as the L1C files"
        )
        # add quicklook/composite to the directory name
        args.save_path = os.path.join(
            os.path.dirname(args.l1c_dir), "quicklook/composite"
        )
    ...
    saveFileName = os.path.join(
        args.save_path,
        "rgb_composite_viewIdx-%s-%s-%s.png"
        % (str(viewIndex[0]), datetime.now().strftime("%Y-%m-%d"), args.tag),
    )
