...
def exportData2CSV(data_dict, var2export="image_0", idx=6, filename="gapmap"):
    # export data to csv
...
def projectVar(
    ...
    **kwargs,
    ...
    # project image
    im = plt.contourf(lon, lat, data2plot, 600, transform=ccrs.PlateCarree(), **kwargs)
    cax = fig.add_axes(
        [
    ...
    if saveImage:
        fig.savefig(
            "gapmap%s_%s_%s.png" % (var2plot, idx, filename),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
