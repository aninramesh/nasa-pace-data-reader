...
def plotL1C(args, fig_, rgb_, temp_num=0, viewIndex=[36, 4, 84]):
    ...
    try:
        ...
        assert (
            -180 <= args.fixed_lat <= 180
        ), "Error: fixed_lat must be between -180 and 180"
        ...
        (
            ...
            rgb_["dateline"][l1c_file],
            rgb_["lon_center"][l1c_file],
            rgb_["lat_center"][l1c_file],
        ) = plt_.projectedRGB(
            proj="Orthographic",
            viewAngleIdx=viewIndex,
            normFactor=normFactor,
            scale=scale_,
            saveFig=True,
            returnRGB=True,  # type: ignore
            figsize=(6, 6),
            noShow=True,
            ...
            for key in rgb_["rgb_new"]:
                if rgb_["dateline"][key]:
                    ax2.imshow(
                        rgb_["rgb_new"][key],
                        origin="lower",
                        extent=rgb_["rgb_extent"][key],
                        transform=ccrs.PlateCarree(
                            central_longitude=rgb_["lon_center"][key]
                        ),
                    )
                else:
                    ax2.imshow(
...
def makeMovieFromImages(movie_dir, movie_name="movie", px=1600, codec="mpeg4"):
    ...
    if codec == "mpeg4":
        os.system(
            f"ffmpeg -r 1 -i {movie_dir}/%04d.png -vcodec mpeg4 -crf 0 -y {movie_dir}/{movie_name}.mp4"
        )
        os.system(
            f"ffmpeg -r 1 -i {movie_dir}/%04d_seq.png -vcodec mpeg4 -crf 0 -y {movie_dir}/{movie_name}_seq.mp4"
        )
    elif codec == "libx264":
        os.system(
            f"ffmpeg -r 1 -i {movie_dir}/%04d.png -vcodec libx264  -y {movie_dir}/{movie_name}.mp4"
        )
        os.system(
            f"ffmpeg -r 1 -i {movie_dir}/%04d_seq.png -vcodec libx264 -y {movie_dir}/{movie_name}_seq.mp4"
        )
    elif codec == "h264":
        os.system(
            f"ffmpeg -r 1 -i {movie_dir}/%04d.png -c libx264 -crf 18 -preset ultrafast -pix_fmt yuv420p -y {movie_dir}/{movie_name}.mp4"
        )
        os.system(
            f"ffmpeg -r 1 -i {movie_dir}/%04d_seq.png -c libx264 -crf 18 -preset ultrafast -pix_fmt yuv420p -y {movie_dir}/{movie_name}_seq.mp4"
        )
...
if os.system("which convert") != 0:
    ...
    print(
        "Please install imagemagick using the following command (NyX): spack load imagemagick"
    )
...
# -- optional arguments
parser.add_argument(
    "--dpi", type=int, required=False, default=400, help="DPI of the saved figure"
)
parser.add_argument(
    "--movie-only", type=bool, required=False, default=False, help="Create movie only"
)
parser.add_argument(
    "--normFactor",
    type=int,
    required=False,
    default=400,
    help="Normalization factor for the RGB image",
)
parser.add_argument(
    "--viewIndex",
    type=int,
    required=False,
    default=0,
    help="Viewing angle for the RGB image option: 0 = -8, 1 = -43, 2 = ~54, 3 = ~22",
)
parser.add_argument(
    "--fixed_lat",
    type=int,
    required=False,
    default=0,
    help="Fixed latitude for the orthographic projection",
)
parser.add_argument(
    "--fixed_lon",
    type=int,
    required=False,
    default=0,
    help="Fixed longitude for the orthographic projection",
)
parser.add_argument(
    "--time_range",
    type=bool,
    required=False,
    default=0,
    help="Time range for the L1C files",
)
...
# movie directory
movie_dir = (
    f"{l1c_dir}/movie_{str(args_.viewIndex)}" + "_Idx_lat" + str(args_.fixed_lat)
)
...
try:
    (
        makeMovieFromImages(
            movie_dir=movie_dir, movie_name="FullOrbit", px=1600, codec="h264"
        )
        if bool(args_.movie_only)
        else None
    )
except Exception as e:
    print(f"Error: {e}")
    print(
        "No need to worry, the images are saved in the directory, you can create the movie using the following command:"
    )
