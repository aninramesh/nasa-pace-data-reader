...
class Plot:
    ...
    def setBand(self, band, verbose=True):
        ...
        if self.instrument == "OCI":
            if self.band == "blue":
                self.wavIndex = np.argmin(
                    np.abs(self.data["intensity_wavelength"] - 440)
                )
            elif self.band == "all":
                self.wavIndex = range(0, self.data["intensity_wavelength"].shape[1])
    ...
    def plotPixel(
        ...
        **kwargs,
        ...
        ], "Invalid x-axis variable"
        xData_, dataVar_, unit_ = self.physicalQuantity(
            x, y, dataVar, xAxis=xAxis, maskFlag=maskFlag
        )
    ...
    def physicalQuantity(
        self, x=None, y=None, dataVar="i", xAxis="scattering_angle", maskFlag=True
    ):
        ...
        if self.instrument == "HARP2":
            if (
                self.reflectance
                and dataVar.lower() in self.vars2plot
                and dataVar.lower() not in ["dolp"]
            ):
                # πI/F0
                dataVar_ = (
                    self.data[dataVar][x, y, self.bandAngles, self.wavIndex]
                    * np.pi
                    / self.data["F0"][self.bandAngles, self.wavIndex]
                )
                unit_ = ""
            else:
                dataVar_ = self.data[dataVar][x, y, self.bandAngles, self.wavIndex]
                unit_ = (
                    "(" + self.data["_units"][dataVar] + ")"
                    if not dataVar == "dolp"
                    else ""
                )
            ...
            if self.reflectance and dataVar in self.vars2plot:
                ...
                if self.bandAngles is not None:
                    for i in self.bandAngles:
                        if self.data[dataVar] is not None and not np.all(
                            np.ma.getmask(
                                self.data[dataVar][
                                    x, y, self.bandAngles[i], self.wavIndex
                                ]
                            )
                        ):
                            if maskFlag:
                                dataVar_ = (
                                    ...
                                    * np.pi
                                    / self.data["F0"][self.bandAngles[i], self.wavIndex]
                                )
                            else:
                                dataVar_ = (
                                    np.ma.getdata(
                                    ...
                                    * np.pi
                                    / np.ma.getdata(
                                        self.data["F0"][
                                            self.bandAngles[i], self.wavIndex
                                        ]
                                    )
                                )
                            continue
                ...
                if "dataVar_" not in locals():
                    print(
                        f"...No valid data found for the variable {dataVar} at the pixel ({x}, {y})"
                    )
                    # end the script
            ...
            if (
                self.reflectance
                and dataVar in self.vars2plot
                and dataVar.lower() not in ["dolp"]
            ):
                # πI/F0
                dataVar_ = (
                    self.data[dataVar][x, y, :, waveIndex].T
                    * np.pi
                    / self.data["F0"][:, waveIndex]
                )
                unit_ = ""
            else:
    ...
    def setFigure(self, figsize=(10, 5), **kwargs):
        ...
        if self.plotAll:
            ...
            print(
                f"...Setting the subplots with number of bands {len(self.bands2plot)} and number of variables {len(self.vars2plot)}"
            )
            fig_, ax_ = plt.subplots(
                nrows=len(self.vars2plot),
            ...
            for j, band in enumerate(self.bands2plot):
                ...
                if axisLabel and j == 0:
                    if unit_ and showUnit:
                        ...
                        (
                            axAll[i, j].set_ylabel(r"R$_%s$" % vars)
                            if not vars == "dolp"
                            else axAll[i, j].set_ylabel(vars)
                        )
                    else:
    ...
    def plotRGB(
        ...
        **kwargs,
        ...
        rgb = np.zeros(
            (self.data[var].shape[0], self.data[var].shape[1], 3), dtype=np.float16
        )
        ...
        if self.instrument == "HARP2":
            if rgb_dolp:
                rgb[:, :, 0] = (
                    self.data["i"][:, :, idx[0], 0] * self.data[var][:, :, idx[0], 0]
                )
                rgb[:, :, 1] = (
                    self.data["i"][:, :, idx[1], 0] * self.data[var][:, :, idx[1], 0]
                )
                rgb[:, :, 2] = (
                    self.data["i"][:, :, idx[2], 0] * self.data[var][:, :, idx[2], 0]
                )
            else:
            ...
            if var == "dolp":
                rgb[:, :, 0] = (
                    self.data["i"][:, :, idx[0], idxR]
                    * self.data[var][:, :, idx[0], idxR]
                )
                rgb[:, :, 1] = (
                    self.data["i"][:, :, idx[1], idxG]
                    * self.data[var][:, :, idx[1], idxG]
                )
                rgb[:, :, 2] = (
                    self.data["i"][:, :, idx[2], idxB]
                    * self.data[var][:, :, idx[2], idxB]
                )
            else:
                if self.instrument == "SPEXone":
        ...
        if plot:
            ...
            if self.instrument == "HARP2":
                ...
                plt.title(
                    f'{self.data["date_time"]} \nRGB image of the instrument {self.instrument}\n using "{var}" variable at angle {viewAngleIdx[0]}',
                    fontsize=8,
                )
            elif self.instrument == "SPEXone":
                plt.title(
    ...
    def projectVar(
        ...
        **kwargs,
        ...
        assert var in self.data.keys(), (
            "Invalid variable, use one of the available variables %s" % self.data.keys()
        )
        ...
        if viewAngleIdx is None:
            ...
            viewAngleIdx = np.argmin(
                np.abs(self.data["view_angles"][self.bandAngles] - viewAngle)
            )
        ...
        if proj == "PlateCarree":
            ...
            gl.xlocator = mticker.FixedLocator(
                np.around(np.linspace(np.nanmin(lon), np.nanmax(lon), 6), 2)
            )
        ...
        if ax is not None:
            ...
            if not highResStockImage:
                (
                    ax.stock_img()
                    if stockImage
                    else ax.add_feature(cfeature.OCEAN, zorder=0)
                )
            ...
            ax.add_feature(
                cfeature.COASTLINE, edgecolor="black", linewidth=1, alpha=0.5
            )
            (
                ax.add_feature(
                    cfeature.LAKES, edgecolor="black", linewidth=1, alpha=0.5
                )
                if lakes
                else None
            )
            (
                ax.add_feature(
                    cfeature.RIVERS, edgecolor="black", linewidth=1, alpha=0.5
                )
                if rivers
                else None
            )
        ...
        if level.lower() == "l1b":
            ...
            plt.contourf(lon, lat, data_, 60, transform=ccrs.PlateCarree(), **kwargs)

        else:
            if self.reflectance and var in ["i", "q", "u"]:
                # πI/F0
                data_ = (
                    self.data[var][:, :, viewAngleIdx, 0]
                    * np.pi
                    / self.data["F0"][viewAngleIdx, 0]
                )
            else:
        ...
        var, unit_ = (
            self.reflectanceChange(var)
            if self.reflectance
            else (var, self.data["_units"][var])
        )
        # var = '%s_' %var
        ...
        if ax is not None:
            ...
            ax.set_title(
                f"${var}{{({self.band})}}$ at {round(float(viewAngle), 2)}° viewing angle \nof the instrument {self.instrument}"
            )
            plt.show()
    ...
    def projectedRGB(
        ...
        **kwargs,
        ...
        Returns:
            None
        """
        assert proj.lower() in [
            "platecarree",
            "orthographic",
            "none",
        ], "Invalid projection method currently only PlateCarree and Orthographic are supported"
        ...
        print(
            f"...Centering the projection at lon:{round(lon_center, 3)}, lat:{round(lat_center,3)}"
        )
        ...
        if transitionFlag:
            ...
            rgb_new, rgb_extent = self.GridRGB(
                lon, lat, dateline=True, proj_size=proj_size
            )

        else:
            # rgb_new, rgb_extent = self.GridRGB(lon, lat, dateline=False, proj_size=proj_size)
            rgb_new, nlon, nlat = self.meshgridRGB(
                lon, lat, return_mapdata=False, proj_size=proj_size
            )  # Created projection image
        ...
        if ax is None and proj.lower() != "none":
            ...
            if figsize is None:
                fig = (
                    plt.figure(figsize=(4, 5), dpi=self.plotDPI) if fig is None else fig
                )
        ...
        if proj == "Orthographic":
            ...
            ax.add_feature(
                cfeature.COASTLINE, edgecolor="black", linewidth=0.2, alpha=0.5
            )
            ...
            gl.ylabels_right = False
            gl.xlocator = mticker.FixedLocator(
                np.around(np.linspace(rgb_extent[0], rgb_extent[1], 6), 2)
            )
            gl.xformatter = LONGITUDE_FORMATTER
            gl.yformatter = LATITUDE_FORMATTER
        ...
        if proj.lower() != "none":
            ...
            if saveFig:
                location = (
                    f"./{self.instrument}_RGB_{str(viewAngleIdx[0])}_proj_{proj}.png"
                )
    ...
    def meshgridRGB(self, LON, LAT, proj_size=(900, 400), return_mapdata=False):
        ...
        newrr = interpolate.griddata(
            (LON.ravel(), LAT.ravel()),
            rr.ravel(),
            (newxx.ravel(), newyy.ravel()),
            method="nearest",
            fill_value=0,
        )
        newgg = interpolate.griddata(
            (LON.ravel(), LAT.ravel()),
            gg.ravel(),
            (newxx.ravel(), newyy.ravel()),
            method="nearest",
        ...
        rgb_proj = np.zeros(
            [np.shape(newrr)[0], np.shape(newrr)[1], 3], dtype=np.float32
        )
    ...
    def GridRGB(self, lon, lat, dateline=True, proj_size=(900, 400)):
        ...
        new_lon, new_lat = np.meshgrid(
            np.linspace(lon_min, lon_max, proj_size[1]),
        ...
        if (
            dateline
        ):  # if the dateline is present, again shift the values to be continuous
            tmp_lon[tmp_lon < 0] = 360 + tmp_lon[tmp_lon < 0]
        else:  # else do nothing
            tmp_lon = tmp_lon
        points = np.vstack(
            [tmp_lon, lat.flatten()]
        ).T  # combine the lon and lat arrays into a 2D array according griddate spec
        new_points = np.vstack(
            [new_lon.flatten(), new_lat.flatten()]
        ).T  # same for the new grid
        ...
        new_values = interpolate.griddata(
            points, values, new_points, method="nearest"
        )  # do interpolation (takes a few seconds)
        # Reshape the interpolated values to match the shape of the new grid
        new_r = new_values.reshape(
            new_lon.shape
        )  # reshape result back to meshgrid expectations
        ...
        tmp_alpha = (
            np.ones_like(new_b, dtype="float32") * 1
        )  # create an alpha channel (opacity) that is 1 (fully opaque) everywhere
        new_rgb = np.stack(
            [new_r, new_g, new_b, tmp_alpha], axis=-1
        )  # stack into a new RGB on the last axis

        new_rgb[new_rgb[:, :, 0] <= 0, -1] = (
            0  # set the alpha channel to 0 where the RGB is invalid making it full transparent
        )
        ext = [
            lon_min,
            lon_max,
            lat_min,
            lat_max,
        ]  # package the edges of the new retangular grid to give to imshow
