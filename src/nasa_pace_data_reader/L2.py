...
class L2:
    ...
    def setInstrument(self, instrument: str = None) -> None:
        """Sets the instrument.
    ...
    def projectVar(
        ...
        var: str,
        wavelength: int = None,
        proj: str = "PlateCarree",
        dpi: int = 300,
        noAxisTicks: bool = False,
        ...
        **kwargs,
        ...
        if wavelength is None:
            ...
            assert (
                wavelength in self.l2_dict["wavelengths"]
            ...
            if "vmax" in kwargs or "vmin" in kwargs:
                ...
                if "vmax" in kwargs and "vmin" in kwargs:
                    if limitTriangle[0] and limitTriangle[1]:
                        plt.colorbar(
                            im, cax=ax_cb, orientation=orientation, extend="both"
                        )
                    elif limitTriangle[0]:
                        plt.colorbar(
                            im, cax=ax_cb, orientation=orientation, extend="min"
                        )
                    elif limitTriangle[1]:
                        plt.colorbar(
                            im, cax=ax_cb, orientation=orientation, extend="max"
                        )
            else:
        ...
        if var not in ["chi2", "n_iter", "quality_flag"]:
            plt.colorbar(im, cax=ax_cb, orientation=orientation).set_label(
                self.var_units[var]
            )
        ...
        if saveFig:
            if savePath is None:
                fig.savefig(
                    f"{var}_wavelength_{wavelength}_nm.png", dpi=dpi, transparent=True
                )
            else:
                ...
                full_path = os.path.join(
                    savePath, f"{var}_wavelength_{wavelength}_nm.png"
                )
